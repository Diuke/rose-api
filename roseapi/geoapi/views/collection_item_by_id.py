...
def collection_item_by_id(request: HttpRequest, collectionId: str, featureId: int):
    ...
    if f in utils.F_GEOJSON:
        ...
        try:
            item_obj = collection_model.objects.get(pk=item_feature_id)
        except collection_model.DoesNotExist:
            return responses.response_not_found_404(f"Feature {featureId} not found.")

        properties = {field: getattr(item_obj, field, "") for field in fields}
        ...
        return render(request, "collections/item_detail.html", context)
    else:
